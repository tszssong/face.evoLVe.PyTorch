...
configurations = {
    1: dict(
        ...
        DATA_ROOT = '/data3/zhengmeisong/data/ms1m_ali111', # the parent root where your train/val/test data are stored
        ...
        LOSS_NAME = 'Triplet',   # support: ['Focal', 'Softmax', 'Triplet']
        ...
        LR = 0.1, # initial LR
        NUM_EPOCH = 12500, # total epoch number (use the firt 1/25 epochs to warm up)
        WEIGHT_DECAY = 5e-4, # do not apply to batch_norm parameters
        MOMENTUM = 0.9,
        STAGES = [135, 465, 895], # epoch stages to decay learning rate
        #STAGES = [35, 65, 95], # epoch stages to decay learning rate

        DEVICE = torch.device("cuda:3" if torch.cuda.is_available() else "cpu"),
        MULTI_GPU = True, # flag to use multiple GPUs; 
        GPU_ID = [3], # specify your GPU ids
        PIN_MEMORY = True,
        NUM_WORKERS = 16,
    ),
}
