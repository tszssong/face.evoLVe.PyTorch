...
def _interpolate(raw, input,size=None, scale_factor=None, mode='nearest', align_corners=None):
    # 定义的参数包�?scale,即输出与输入的尺寸比�?�?2;scale_h、scale_w,
    # �?scale,分别�?h、w 方向上的尺寸比例;pad_out_h、pad_out_w,仅在 scale �?2 �?    # 有用,对输出进行额�?padding �?h、w 方向上的数�?upsample_h、upsample_w,�?    # 出图像尺寸的数值。在 Upsample 的相关代码中,推荐仅仅使用 upsample_h�?    # upsample_w 准确定义 Upsample 层的输出尺寸,其他所有的参数都不推荐继续使用�?    # for nearest _interpolate
...
# 核心组件，通过该类，实现对torch的function中的operators的输入，输出以及参数的读�?
class Rp(object):
