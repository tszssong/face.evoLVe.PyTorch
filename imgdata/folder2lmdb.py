...
def match_label(idx2img, img2lab):
    idx2label = {}
    for key in idx2img.keys():
        idx2label[key] = img2lab[idx2img[key]]
    return idx2label
...
class ImageFolderLMDB(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im2arr = np.array(img)
        # print(im2arr.shape)
        target = self.idx2label[self.keys[index].decode("ascii")]
        target = int(target)
        # print(target)
