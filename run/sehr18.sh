...
python myTrainDali.py  --backbone-name $MODEL \
  ...
  --backbone-resume-root /ai_data/zhengmeisong/openModels/hrnetv2_w18_imagenet_pretrained.pth \
  --backbone-name $MODEL \
  --disp-freq 1000 --batch-size 400 \
  --lr 0.01 --lr-stages 4,8,12,16 --num-epoch 20 \
  --loss-name Focal --head-name Combine \
  --model-root ../py-model/${MODEL}/ \
  --num-classes 143474 \
  --gpu-ids 7,6 2>&1 | tee ../py-log/${MODEL}_`date +'%m_%d-%H_%M'`.log
